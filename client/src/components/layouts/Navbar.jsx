...
const PAGE_META = {
  ...
  '/filter':      { title: 'Filter & Ekspor',sub: 'Saring data berdasarkan kriteria' },
...
const NOTIFS = [
  { id: 1, type: 'alert',   text: 'Pengeluaran makanan melebihi budget', time: '5 menit lalu',  unread: true },
  { id: 2, type: 'success', text: 'Pemasukan Rp 3.500.000 berhasil dicatat', time: '1 jam lalu',   unread: true },
  { id: 3, type: 'info',    text: 'Laporan bulanan Desember siap diunduh', time: '3 jam lalu',   unread: false },
]

// ─── Period options ────────────────────────────────────────────────────────────
const PERIODS = ['Jan – Mar 2025', 'Apr – Jun 2025', 'Jul – Sep 2025', 'Okt – Des 2025']
    ...
    ">
      ...
      <div className="flex items-center gap-3">
        ...
        <div ref={periodRef} className="relative">
          <button
            onClick={() => { setShowPeriod(p => !p); setShowNotifs(false); setShowProfile(false) }}
            className="
              flex items-center gap-2 px-3 py-2 rounded-lg
        ...
        <div ref={notifRef} className="relative">
          <button
            onClick={() => { setShowNotifs(n => !n); setShowPeriod(false); setShowProfile(false) }}
            className="
          ...
          {showNotifs && (
            ...
            ">
              <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
                <span className="text-[13px] font-semibold text-slate-200">Notifikasi</span>
                <button
                  onClick={markAllRead}
        ...
        <div ref={profileRef} className="relative">
          <button
            onClick={() => { setShowProfile(p => !p); setShowNotifs(false); setShowPeriod(false) }}
          ...
          >
            ...
            <div className="hidden sm:block text-left">
              <p className="text-[12px] font-semibold text-slate-200 leading-none">Rafi Ahmad</p>
          ...
          {showProfile && (
            ...
            ">
              ...
              <div className="px-4 py-3 border-b border-white/[0.06]">
                <p className="text-[13px] font-semibold text-slate-200">Rafi Ahmad</p>
                <p className="text-[11px] text-slate-500 mt-0.5">rafi@email.com</p>
              </div>
              {/* Menu items */}
              ...
              <div className="border-t border-white/[0.06] py-1.5">
                <ProfileMenuItem icon={<LogoutIcon />} label="Keluar" danger />
              </div>
            </div>
          )}
        </div>
...
// ─── Sub-components ───────────────────────────────────────────────────────────
function ProfileMenuItem({ icon, label, danger }) {
  return (
    <button className={`
