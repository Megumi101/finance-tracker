...
function formatRupiah(v) {
  ...
  if (abs >= 1000000) return (v > 0 ? '+' : '') + 'Rp ' + (abs / 1000000).toFixed(1) + ' Jt'
  if (abs >= 1000)    return (v > 0 ? '+' : '-') + 'Rp ' + (abs / 1000).toFixed(0) + ' Rb'
  return (v > 0 ? '+' : '') + 'Rp ' + v
}
...
function Initials({ nama, tipe }) {
  ...
  const bg = tipe === 'masuk' ? 'bg-violet-600/15 text-violet-400' : 'bg-slate-700/50 text-slate-400'
  return (
    <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-[12px] font-bold flex-shrink-0 ${bg}`}>
      {letters}
    </div>
  )
}
    ...
    <div className="bg-[#0C1120] border border-white/[0.06] rounded-2xl p-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-[12px] text-slate-500 mt-0.5">{recentTransactions.length} transaksi terakhir</p>
        </div>
      ...
      <div className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-2 pb-2 border-b border-white/[0.05]">
        {['Transaksi', 'Tanggal', 'Jumlah', 'Status'].map(h => (
          <span key={h} className="text-[10px] font-medium tracking-[1.2px] uppercase text-slate-600">{h}</span>
        ))}
      </div>
            ...
            >
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-[13px] font-medium text-slate-200 truncate">{txn.nama}</p>
                  <p className="text-[11px] text-slate-500 mt-0.5">{txn.kategori}</p>
                </div>
              </div>

              {/* Tanggal */}
              <span className="text-[12px] text-slate-500 whitespace-nowrap">{txn.tanggal}</span>

              {/* Jumlah */}
              <span className={`text-[13px] font-mono font-bold whitespace-nowrap ${isPositive ? 'text-emerald-400' : 'text-red-400'}`}>
                {formatRupiah(txn.jumlah)}
              </span>

              {/* Status */}
              <span className={`text-[11px] font-medium px-2.5 py-1 rounded-full whitespace-nowrap ${status.cls}`}>
                {status.label}
              </span>
            </div>
          )
