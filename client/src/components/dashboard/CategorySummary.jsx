...
import { kategoris } from '../../data/dashboardData'
...
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div className="bg-[#111827] border border-white/10 rounded-xl px-3 py-2 shadow-2xl">
      <p className="text-[12px] font-semibold text-slate-200">{d.emoji} {d.nama}</p>
      <p className="text-[11px] text-slate-400 mt-0.5">{formatRupiah(d.jumlah)}</p>
    </div>
  )
}
...
export default function CategorySummary() {
  ...
  const total = kategoris.reduce((sum, k) => sum + k.jumlah, 0)
  ...
  return (
    <div className="bg-[#0C1120] border border-white/[0.06] rounded-2xl p-6 flex flex-col">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-[15px] font-semibold text-slate-100">Ringkasan Kategori</h2>
          <p className="text-[12px] text-slate-500 mt-0.5">Pengeluaran bulan ini</p>
        </div>
      ...
      <div className="relative w-full h-[160px] flex items-center justify-center mb-4">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          {hovered !== null ? (
            <>
              <span className="text-[18px]">{kategoris[hovered].emoji}</span>
              <span className="text-[11px] text-slate-400 mt-0.5">{kategoris[hovered].nama}</span>
            </>
          ) : (
            <>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-center mb-1">
                <span className="text-[12px] font-medium text-slate-300">{k.nama}</span>
                <span className="text-[11px] text-slate-500">{k.transaksi} transaksi</span>
              </div>
              <div className="w-full h-[3px] bg-white/[0.05] rounded-full overflow-hidden">
                <div
                  className="h-full rounded-full transition-all duration-500"
                  style={{ width: k.persen + '%', background: k.warna }}
                />
              </div>
            </div>
