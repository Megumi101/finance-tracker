    ...
    <div className="bg-[#111827] border border-white/10 rounded-xl px-4 py-3 shadow-2xl">
      ...
      {payload.map((entry) => (
        <div key={entry.dataKey} className="flex items-center gap-2 mb-1 last:mb-0">
          ...
          />
          <span className="text-[12px] text-slate-400 capitalize w-24">{entry.dataKey}</span>
  ...
  return (
    <div className="bg-[#0C1120] border border-white/[0.06] rounded-2xl p-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-[15px] font-semibold text-slate-100">Grafik Keuangan</h2>
          <p className="text-[12px] text-slate-500 mt-0.5">Pemasukan, pengeluaran & tabungan</p>
        </div>
        <div className="flex items-center gap-1 bg-[#080C14] rounded-xl p-1 border border-white/[0.05]">
          <PeriodBtn label="Harian"  active={period === 'harian'}  onClick={() => setPeriod('harian')} />
          <PeriodBtn label="Bulanan" active={period === 'bulanan'} onClick={() => setPeriod('bulanan')} />
        </div>
      </div>
