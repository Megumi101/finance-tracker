...
const labelCls = 'block text-[11px] font-medium text-slate-500 uppercase tracking-[1px] mb-2'
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/[0.06]">
          <div>
            ...
            <p className="text-[12px] text-slate-500 mt-0.5">
              {isEdit ? 'Perbarui detail transaksi' : 'Isi detail transaksi baru'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { value: 'masuk',  label: '↑ Pemasukan',  active: 'bg-emerald-600/15 border-emerald-500/40 text-emerald-400' },
                { value: 'keluar', label: '↓ Pengeluaran', active: 'bg-red-600/15 border-red-500/40 text-red-400' },
              ].map(opt => (
                <button
                  key={opt.value}
          ...
          <div>
            ...
            />
            {errors.nama && <p className="text-[11px] text-red-400 mt-1">{errors.nama}</p>}
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[13px] text-slate-500">Rp</span>
              <input
                type="number"
                placeholder="0"
                value={form.jumlah}
                onChange={e => set('jumlah', e.target.value)}
                className={inputCls + ' pl-10 ' + (errors.jumlah ? 'border-red-500/50' : '')}
                min="0"
              />
            </div>
            {errors.jumlah && <p className="text-[11px] text-red-400 mt-1">{errors.jumlah}</p>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="relative">
                ...
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 pointer-events-none text-[10px]">▾</span>
              </div>
            </div>
            <div>
              ...
              <input
                ...
                className={inputCls + (errors.tanggal ? ' border-red-500/50' : '')}
                style={{ colorScheme: 'dark' }}
              />
              {errors.tanggal && <p className="text-[11px] text-red-400 mt-1">{errors.tanggal}</p>}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { value: 'berhasil', label: 'Berhasil', cls: 'border-emerald-500/40 text-emerald-400 bg-emerald-600/10' },
                { value: 'tertunda', label: 'Tertunda', cls: 'border-amber-500/40 text-amber-400 bg-amber-600/10' },
                { value: 'gagal',    label: 'Gagal',    cls: 'border-red-500/40 text-red-400 bg-red-600/10' },
              ].map(opt => (
                <button
                  key={opt.value}
          ...
          {/* Catatan */}
          <div>
            <label className={labelCls}>Catatan <span className="normal-case text-slate-700">(opsional)</span></label>
            <textarea
