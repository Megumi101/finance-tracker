...
function exportCSV(data) {
  const header = ['ID', 'Nama', 'Kategori', 'Tipe', 'Jumlah', 'Tanggal', 'Status', 'Catatan']
  const rows = data.map(t => [
    t.id, t.nama, t.kategori, t.tipe, t.jumlah, t.tanggal, t.status, t.catatan
  ])
...
function SummaryBar({ data }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      {[
        { label: 'Total Pemasukan', value: totalMasuk,  color: 'text-emerald-400', bg: 'bg-emerald-500/5 border-emerald-500/10' },
        { label: 'Total Pengeluaran', value: totalKeluar, color: 'text-red-400',     bg: 'bg-red-500/5 border-red-500/10' },
        { label: 'Selisih',          value: saldo,       color: saldo >= 0 ? 'text-violet-400' : 'text-red-400', bg: 'bg-violet-500/5 border-violet-500/10' },
      ].map(item => (
        <div key={item.label} className={`rounded-2xl p-4 border ${item.bg}`}>
          ...
          <p className={`text-[20px] font-mono font-bold ${item.color}`}>{formatRupiah(Math.abs(item.value))}</p>
          <p className="text-[11px] text-slate-600 mt-1">{data.length} transaksi</p>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="flex items-center justify-between px-2">
      <p className="text-[12px] text-slate-500">
        Menampilkan {Math.min((page - 1) * perPage + 1, total)}–{Math.min(page * perPage, total)} dari {total} transaksi
      </p>
      <div className="flex items-center gap-1">
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1)
          ...
          .map((p, i) =>
            p === '...'
              ? <span key={i} className="text-slate-600 px-1 text-[12px]">···</span>
              : <PageBtn key={p} label={p} active={p === page} onClick={() => onPage(p)} />
          )
        }
...
// ─── MAIN PAGE ────────────────────────────────────────────────────────────────
const EMPTY_FILTERS = { search: '', tipe: '', kategori: '', status: '', dari: '', sampai: '' }
const PER_PAGE = 8
...
export default function Transaksi() {
  ...
  const filtered = useMemo(() => {
    let d = [...data]
    if (filters.search)   d = d.filter(t => t.nama.toLowerCase().includes(filters.search.toLowerCase()))
  ...
  const sorted = useMemo(() => {
    return [...filtered].sort((a, b) => {
      ...
      if (sort.key === 'jumlah') { va = Number(va); vb = Number(vb) }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[20px] font-bold text-slate-100 tracking-tight">Semua Transaksi</h1>
          <p className="text-[12px] text-slate-500 mt-0.5">{data.length} transaksi tercatat</p>
        </div>
          ...
          <div className="relative">
            ...
            {showExport && (
              <div className="absolute right-0 top-full mt-2 w-40 bg-[#111827] border border-white/[0.08] rounded-xl shadow-2xl py-1.5 z-10">
                <button
                  onClick={() => { exportCSV(sorted); setShowExport(false) }}
                ...
                <button
                  onClick={() => { exportPDF(sorted); setShowExport(false) }}
                  className="w-full text-left px-4 py-2.5 text-[12px] text-slate-400 hover:text-slate-200 hover:bg-white/[0.04] transition-colors"
                >
                  🖨️ Export PDF
                </button>
      ...
      <TransaksiTable
        ...
        sort={sort}
        onSort={handleSort}
        onEdit={(txn) => { setEditData(txn); setModalOpen(true) }}
